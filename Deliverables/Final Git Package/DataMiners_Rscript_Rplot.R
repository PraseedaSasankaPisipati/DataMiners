...
summary(Health_data$Expected.Deaths)  
summary(Health_data$Potentially.Excess.Deaths)  
summary(Health_data$Observed.Deaths)  
...
sd_expected_deaths = sd(Health_data$Expected.Deaths)  
sd_excess_deaths = sd(Health_data$Potentially.Excess.Deaths)  
sd_observed_deaths = sd(Health_data$Observed.Deaths)  

##### Standard Deviation of expected deaths, excess deaths, observed deaths data grouped by State  
sd_expected_deaths_by_state = aggregate( Expected.Deaths ~ State, Health_data, mean )  
sd_excess_deaths_by_state = aggregate( Potentially.Excess.Deaths ~ State, Health_data, mean)  
sd_observed_deaths_by_state = aggregate(Observed.Deaths ~ State, Health_data, mean)  

##### Anova table
Anova.aov= aov(Health_data$Expected.Deaths~ Health_data$Cause.of.Death)  
Anova.aov  
summary(Anova.aov)  

##### Linear Regression
Regression<-lm(Health_data$Expected.Deaths ~ Health_data$Cause.of.Death + Health_data$Population)  
summary(Regression)  
